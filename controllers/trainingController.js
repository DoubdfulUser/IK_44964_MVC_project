...
exports.addTraining = async (req, res) => {
    try {
        const { name, type, intensity, duration, city, location, date, description } = req.body;
...
exports.searchTraining = async (req, res) => {
    try {
        ...
        if (search) {
            const regex = new RegExp(search, 'i'); // 'i' делает поиск нечувствительным к регистру
        ...
        res.render('trainings', { title: 'Trainings', trainings });
    } catch (error) {
        console.error('Error fetching trainings:', error);
        res.status(500).send('Server error');
    }
